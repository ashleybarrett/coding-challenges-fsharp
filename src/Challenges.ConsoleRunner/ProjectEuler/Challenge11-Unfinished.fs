module ProjectEulerChallenge11
...
let solution =
    ...
    let getLargestProduct windowSize row =
        row 
    ...
    let leftToRight = 
        grid
        |> Seq.map(fun x -> getLargestProduct windowSize x)

    let rightToLeft = 
        grid
        |> Seq.map(fun x -> x |> Seq.rev |> getLargestProduct windowSize)

    let topToBottom = 
        {0..maxGridIndex}
        |> Seq.map(fun x -> grid |> Seq.map(fun r -> r.[x]))
        |> Seq.map(fun x -> x |> Seq.rev |> getLargestProduct windowSize)

    let bottomToTop = 
        {0..maxGridIndex}
        |> Seq.map(fun x -> grid |> Seq.map(fun r -> r.[x]) |> Seq.rev)
        |> Seq.map(fun x -> x |> Seq.rev |> getLargestProduct windowSize)
